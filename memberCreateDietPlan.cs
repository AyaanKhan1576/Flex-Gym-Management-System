...
namespace PORJDB1
{
    public partial class memberCreateDietPlan : Form
    {
        ...
        private int GetPlanCount()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 Diet_Plan_ID FROM MEMBER_DIET_PLAN_NEW ORDER BY Diet_Plan_ID DESC", conn))
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("INSERT INTO MEMBER_DIET_PLAN_NEW (Diet_Plan_ID, Plan_Name, Member_ID, Meal_ID, Diet_Goal) VALUES (@DietPlanID, @PlanName, @MemberID, @MealID, @DietGoals)", conn))
                {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    if (row.Cells["MealName"].Value != null) // Check if the row is not empty
                    {
                        ...
                        if (mealID != -1)
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand("INSERT INTO MEMBER_CONSISTS_OF_DIET_NEW2 (Meal_ID, Diet_Plan_ID) VALUES (@MealID, @DietPlanID)", conn))
                            {
        ...
        private void memberCreateDietPlan_Load(object sender, EventArgs e)
        {
